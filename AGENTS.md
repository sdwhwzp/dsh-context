...
- MUST be able to install and work correctly on `@deepseek-ai/dsh` **0.1.5-rc.1+** (the `0.1.5` line from rc.1, the `0.1.7` line from rc.2), with no regressions in runtime dependencies, message parsing, or any user-visible behavior. **`docs/compatibility.md` is the single source of truth for compatibility** — the supported releases, the per-generation event vocabulary, the client seams each line serves, and the runtime baseline gate — so keep that page current and reference it instead of restating its rules here.
- The supported range spans two durable-log generations (V3, V4) that must all keep working from ONE code path; a pre-V3 log is migrated or refused by the RUNNING harness before any plugin read, so the fold never sees legacy spellings. Fold by **SHAPE, never by detected version**, and `src/host/logShapes.ts` is the ONE place the log spellings meet (`firstTokenTimeOfStream`, `replaceRangeOf`, `isTokenChunk`) — `applyTimeline` keeps a single code path over all of them. Never branch fold behavior on `detectHarnessVersion`.
