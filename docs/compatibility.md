...
Last verified: **2026-09-09** (plugin `dsh-context@0.46.1` source tree).
...
| `0.1.5-alpha.1` | V3 | compatible | ✅ baseline `v0.1.5-alpha.1` | ✅ install OK → 1 composed row → uninstall OK → 0 rows |

The automated seam matrix runs for every row on every `pnpm test`. The disposable-profile column is a manual, per-release check: `0.1.2-rc.1` was verified on 2026-09-05 and carried forward; the two newer releases were verified on 2026-09-09 against the official npm registry (a stale mirror can 404 the harness's own dependency closure before the plugin is even considered).
