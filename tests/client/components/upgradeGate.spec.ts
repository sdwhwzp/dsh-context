...
function render(sessionId: string | undefined, extra: { current?: string; minimum?: string } = {}) {
  return mount(h(Gate, { sessionId, current: extra.current ?? '0.1.1-rc.2', minimum: extra.minimum ?? '0.1.5-rc.1' }))
}
...
describe('UpgradeGate', () => {
  test('renders both versions and the upgrade prompt (en and zh)', async () => {
    ...
    try {
      ...
      assert.match(text(en.container), /dsh-context supports DeepSeek Harness v0\.1\.5-rc\.1 or newer\./)
    ...
    const zh = await mount(h(ZhGate, { sessionId: 's-zh', current: '0.1.0', minimum: '0.1.5-rc.1' }))
    try {
      assert.match(text(zh.container), /请更新 DeepSeek Harness 版本/)
      assert.match(text(zh.container), /dsh-context 插件支持 DeepSeek Harness v0\.1\.5-rc\.1 或更新版本。/)
      assert.match(text(zh.container), /v0\.1\.5-rc\.1及更新的版本/)
  ...
  test('an in-place session switch re-evaluates the ledger', async () => {
    ...
    await m.update(h(Gate, { sessionId: 's-switch-b', current: '0.1.0', minimum: '0.1.5-rc.1' }))
    assert.ok(m.container.querySelector('.lc-modal-backdrop') !== null)
    // Switching back to the dismissed session stays closed.
    await m.update(h(Gate, { sessionId: 's-switch-a', current: '0.1.0', minimum: '0.1.5-rc.1' }))
    assert.equal(m.container.querySelector('.lc-modal-backdrop'), null)
    await m.unmount()
  })
