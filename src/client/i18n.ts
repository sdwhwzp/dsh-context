...
export const DICT_ZH: Record<string, string> = {
  ...
  'stats.costTipLedger': '整个会话的累计费用，由 dsh-spend 账本计价：覆盖它已知的全部供应商与模型，按已计费 token 用量、以每次调用自身时间所适用的已发布价目表结算，币种取该部署的设置。',
...
export const DICT_EN: Record<string, string> = {
  ...
  'stats.costTipLedger': 'Cumulative cost of the whole session as the dsh-spend ledger priced it: every provider and model it knows, charged on billed token usage at the published table live at each call\u2019s own time, in the deployment\u2019s configured currency.',
