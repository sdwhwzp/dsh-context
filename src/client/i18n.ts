...
export const DICT_ZH: Record<string, string> = {
  ...
  'stats.costTipLedger': '整个会话的累计费用，由 dsh-spend 账本计价：覆盖它已知的全部供应商与模型，按每次调用自身时间所适用的已发布价目表结算，币种取该部署的设置。',
  'stats.costModelHead': '按模型（费用 · 调用次数）：',
...
export const DICT_EN: Record<string, string> = {
  ...
  'stats.costTipLedger': 'Cumulative cost of the whole session as the dsh-spend ledger priced it: every provider and model it knows, each call charged at the published table live at its own time, in the deployment\u2019s configured currency.',
  'stats.costModelHead': 'By model (cost \u00b7 calls):',
